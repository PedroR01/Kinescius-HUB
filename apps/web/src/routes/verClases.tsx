...
function RouteComponent() {
  ...
  return (
    <main className="min-h-screen bg-white p-10 box-border">
      ...
      {loading && <p className="text-(--green)">Cargando...</p>}

      {!loading && hasLoaded && classes.length === 0 && !error && (
        <p className="text-gray-500 text-sm">No hay clases para mostrar.</p>
      )}
      ...
      {!loading && classes.length > 0 && (
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className="text-left p-2.5 text-(--green) font-semibold text-xs tracking-wide uppercase">
                  Fecha
                </th>
                <th className="text-left p-2.5 text-(--green) font-semibold text-xs tracking-wide uppercase">
                  Hora
