    ...
    <main className="min-h-screen bg-white p-10 box-border">
      ...
      <div className="bg-(--card-style) border border-emerald-150 rounded-2xl p-7 max-w-600 flex flex-col gap-5 mb-12">
        <div className="flex flex-col gap-1">
          ...
          <DatePicker value={fechaFiltro} onChange={setFechaFiltro} placeholder="Cualquier fecha" />
          {fechaFiltro && (
            <p className="text-xs text-gray-500 mt-1">
              {filteredClases.length} clase{filteredClases.length !== 1 ? "s" : ""} en el día
              seleccionado
            </p>
          )}
        </div>
      ...
      {loadingInscriptos && <p className="text-gray-400 text-sm">Cargando inscriptos...</p>}

      {inscriptos.length > 0 && (
        <>
          <p className="text-sm text-gray-500 mb-4">{inscriptos.length} inscripto/s en la clase</p>
              ...
              <thead>
                <tr className="border-b border-emerald-200">
                  {["Nombre", "Apellido", "DNI", "Mail", "Estado"].map((h) => (
                    <th
                      key={h}
                      className="text-left p-2.5 text-(--green) font-semibold text-xs tracking-wide uppercase"
                    >
                      {h}
                    </th>
                  ))}
