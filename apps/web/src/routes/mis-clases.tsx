          ...
          >
            <motion.div variants={fadeUp} transition={EASE_OUT}>
              ...
              <p className="max-w-2xl text-base leading-relaxed text-muted-foreground sm:text-lg">
                Gestioná tus próximos turnos y mantené tu rutina al día. Revisa, cambia o cancela
                tus clases programadas con facilidad.
              </p>
              {mensajeExito && (
        ...
        <div className="mx-auto w-full max-w-5xl">
          <motion.div variants={staggerContainer(0.12)} initial="hidden" animate="visible">
            ...
            {isLoading ? (
              ...
              <motion.div
                variants={fadeUp}
                className="bg-white rounded-3xl p-12 text-center shadow-md"
              >
                <div className="inline-flex h-20 w-20 items-center justify-center rounded-full bg-main/5 text-main mb-6">
                  <Calendar className="w-10 h-10" />
                </div>
                <h3 className="font-heading text-2xl font-bold text-dark-accent mb-3">
                  No tenés clases programadas
                </h3>
                <p className="text-muted-foreground text-lg max-w-md mx-auto">
                  Parece que aún no has agendado ninguna clase. ¡Da el primer paso hacia tu
                  recuperación hoy mismo!
                </p>
              </motion.div>
            ) : activeTab === "HISTORIAL" &&
              historialClases.filter((item) => item.Clase !== null).length === 0 ? (
              <motion.div
                variants={fadeUp}
                className="bg-white rounded-3xl p-12 text-center shadow-md"
              >
        ...
        <div className="fixed inset-0 bg-dark-accent/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all duration-300">
          <CambiarTurno
            ...
            horasOcupadasMismoDia={misClases
              .filter(
                (c) =>
                  c.Clase?.fecha === claseSeleccionada.Clase.fecha &&
                  c.id_clase !== claseSeleccionada.id_clase
              )
