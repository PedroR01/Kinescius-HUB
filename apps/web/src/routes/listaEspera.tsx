...
type ModalData =
  ...
  | {
      type: "all";
      results: { claseId: number; tipo: string | null; index: number; count: number }[];
    };
...
function RouteComponent() {
  ...
  const { classes: clases, loading: loading, error: error } = useClassFetcher();

  const handleViewWaitList = async (clase: KinesciusClass, index: number) => {
    setActionLoading(true);
    try {
      const [countRes, personasRes] = await Promise.all([
        fetch(`${API_BASE}/listaEspera/clase/${clase.id}/count`),
        fetch(`${API_BASE}/listaEspera/clase/${clase.id}`)
      ]);
      ...
      const personas = (await personasRes.json()) as UserProfile[];
    ...
    try {
      const results = await Promise.all(
        clases.map(async (clase, idx) => {
          try {
            const res = await fetch(`${API_BASE}/listaEspera/clase/${clase.id}/count`);
  ...
  return (
    <main
      className={
        "mx-auto grid min-h-svh max-w-[760px] gap-4 px-6 pt-10 pb-16 font-dm-sans text-ks-gray-text antialiased bg-[radial-gradient(ellipse_80%_50%_at_10%_0%,rgba(82,183,136,0.12)_0%,transparent_60%) radial-gradient(ellipse_60%_40%_at_90%_100%,rgba(183,224,85,0.1)_0%,transparent_55%) var(--ks-off-white)] max-sm:px-4 max-sm:pt-5 max-sm:pb-12"
      }
    >
      <BackPreviousRouteButton />
      <section className="ks-hero-card relative overflow-hidden rounded-ks-lg px-10 py-12 shadow-[0_20px_60px_rgba(26,58,42,0.18)] animate-ks-slide-up bg-[linear-gradient(135deg,var(--ks-green-dark)_0%,var(--ks-green-mid)_60%,var(--ks-green-light)_100%)] max-sm:px-6 max-sm:py-8">
      ...
      {!loading && !error && clases.length > 0 && (
        <section className={`${formCardClass} flex flex-wrap items-center justify-between gap-3`}>
      ...
      {loading && (
        <section
          className={`${formCardClass} animate-ks-slide-up rounded-ks-md border border-[rgba(82,183,136,0.25)] px-5 py-4 bg-[linear-gradient(135deg,rgba(82,183,136,0.08),rgba(183,224,85,0.08))]`}
        >
          <p className="text-sm leading-relaxed font-medium text-ks-green-dark">
            Cargando clases...
          </p>
        </section>
      )}
          ...
          <div className="grid gap-3">
            {clases.map((clase, idx) => (
              ...
              >
                ...
                <button
                  className={`${btnBase} ${btnSecondary} whitespace-nowrap`}
                  type="button"
      ...
      {modal && (
        ...
        >
          ...
          >
              ...
              <>
                <div className="mb-4 flex items-start justify-between gap-4 border-b-[1.5px] border-ks-gray-soft pb-4">
                  <div>
                    ...
                    <p className="m-0 text-[13px] text-ks-gray-text">
                      {formatDate(modal.clase.fecha)} · {formatTime(modal.clase.hora)}
                    </p>
                  </div>
                  <button
            ...
            <button
              className={`${btnBase} ${btnPrimary} mt-5 w-full`}
              type="button"
              onClick={() => setModal(null)}
            >
              Cerrar
