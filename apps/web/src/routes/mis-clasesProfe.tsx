...
export const Route = createFileRoute('/mis-clasesProfe')({
...
export default function GestionClases() {
  ...
  const cargarClases = () => {
    ...
    fetch(`${API_BASE}/clases/profesor/mis-clases`, {
      ...
      .then((data) => {
        ...
        const historial = todas
          ...
          .map((c: any) => ({
            id_clase: c.id,
            id_cliente: idCliente,
            estado_historial: c.estado === 1 ? 'Dictada' : 'Cancelada',
            Clase: { id: c.id, fecha: c.fecha, hora: c.hora, tipo: c.tipo, profesor: '', cupo: c.cupo },
          }));
        setMisClases(proximas);
        setHistorialClases(historial);
        setIsLoading(false);
      })
      .catch((err) => {
        console.error("Error al traer clases del profesor:", err);
        setIsLoading(false);
      });
  };
        ...
        <div className="mx-auto w-full max-w-5xl">
          ...
          >
            <div className="flex gap-4 mb-8 border-b border-slate-200">
              <button
                className={`pb-4 px-2 font-semibold text-lg border-b-2 transition-colors ${activeTab === 'PROXIMAS'
              ...
              <button
                className={`pb-4 px-2 font-semibold text-lg border-b-2 transition-colors ${activeTab === 'HISTORIAL'
                  ? 'border-main text-main'
                  : 'border-transparent text-slate-500 hover:text-slate-700'
                  }`}
