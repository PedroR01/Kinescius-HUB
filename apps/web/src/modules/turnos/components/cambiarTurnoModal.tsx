...
import {
  obtenerClasesDisponiblesRequest,
  cambiarTurnoRequest,
...
export const CambiarTurno: React.FC<CambiarTurnoModalProps> = ({
  ...
  return (
    <div className="bg-white p-8 rounded-3xl shadow-xl max-w-md w-full">
      <h2 className="text-2xl font-heading font-extrabold text-dark-accent mb-6">
        Cambiar Horario de Turno
      </h2>

      <div className="mb-6 bg-surface p-5 rounded-2xl text-slate-700">
        <p className="mb-1">
          <strong className="font-semibold text-dark-accent">Actividad:</strong> {actividad}
        </p>
        <p className="mb-1">
          <strong className="font-semibold text-dark-accent">Fecha:</strong> {fechaActual}
        </p>
        <p>
          <strong className="font-semibold text-dark-accent">Hora:</strong> {horaActual.slice(0, 5)}{" "}
          hs
        </p>
      </div>
      ...
      <div className="mb-8">
        ...
        {loadingFetch ? (
          <p className="text-sm font-medium text-slate-500 animate-pulse">
            Buscando alternativas en tiempo real...
          </p>
          ...
          <div className="relative">
            {/* Select Trigger */}
            <div
              className={`w-full rounded-2xl bg-surface px-4 py-3.5 font-semibold cursor-pointer transition-colors flex justify-between items-center ${
                claseNuevaId === "" ? "text-slate-500" : "text-main"
              }`}
              onClick={() => {
            ...
            >
              ...
              <svg
                className={`h-5 w-5 fill-current text-slate-500 transition-transform ${
              ...
              <>
                ...
                <div className="absolute z-20 w-full mt-2 bg-white rounded-2xl shadow-xl overflow-hidden border border-border/30">
                  ...
                  {clasesFiltradas.map((clase) => (
                    ...
                    >
                      {clase.hora.slice(0, 5)} hs — ({clase.actividad} - {clase.cuposDisponibles}{" "}
                      cupos)
                    </div>
                  ))}
                </div>
